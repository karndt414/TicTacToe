        ...
        <CreateRoomScreen 
          ...
          onRoomCreated={async (username: string, roomName: string) => {
            try {
              ...
              subscribeToRoom(room.id, (players) => {
        ...
        <JoinRoomScreen 
          ...
          onRoomJoined={async (username: string, roomCode: string) => {
            try {
              setLoading(true)
              const player = await createPlayer(username)
              const { room, players } = await joinRoom(roomCode, player.id)
              ...
              // Subscribe to room updates
              subscribeToRoom(room.id, (updatedPlayers) => {
        ...
        <LobbyScreen 
          ...
          onStartGame={async () => {
            try {
              ...
              subscribeToGame(game.id, (updatedGame) => {
      ...
      {appState === 'game' && appData.currentGame && (
        <GameScreen 
          ...
          onSquareClick={async (square: number) => {
            try {
              setLoading(true)
              const match = await createMatch(appData.currentGame!.id, square, 'rps')
              setAppData(prev => ({ ...prev, currentMatch: match }))
              setAppState('mini-game')
              
              // Subscribe to match updates
              subscribeToMatch(match.id, (updatedMatch) => {
